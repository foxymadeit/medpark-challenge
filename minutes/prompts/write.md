...
- **Every fact appears exactly once, with its ID, in its command.** Topics in `\agendaitem` and `\topic`; notes in `\noted`; decisions in `\decision`; actions in `\action`; items marked `confirm` only in `\needsconfirmation`.
...
- **Only these commands**, with plain text inside their braces: `\begin{agenda}` `\agendaitem{ID}{title}` `\end{agenda}` `\topic{ID}{title}` `\noted{ID}{sentence}` `\decision{ID}{sentence}{vote}` `\action{ID}{owner}{deadline}{what they will do}` `\needsconfirmation{ID}{item}`. Escape `%` as `\%` and `&` as `\&`. No other backslash, no braces inside text, no Markdown, no comments.

## Do these in order

1. Write the agenda: one `\agendaitem` per topic, in the given order.
2. For each topic: `\topic`, then its notes, then its decisions, then its actions, in the given order.
3. After the last topic, one `\needsconfirmation` per item marked `confirm`.
4. Re-read: every ID exactly once, owners and deadlines copied, nothing added.
...
Plain, exact, formal. Short sentences, past tense for what happened, the language's standard formula for decisions (above). Spell a medical abbreviation out once, then use it. Do not use dashes as punctuation, filler, or summary sentences. Write like the secretary of a hospital board, not like a chatbot.

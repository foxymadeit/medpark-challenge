...
def compile_pdf(tex: str, xmp: str, out_pdf: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="mom-") as tmp:
        ...
        env = {**os.environ, "TEXINPUTS": f"{TEMPLATE.as_posix()}//:", "openout_any": "p", "openin_any": "p",
               "shell_escape": "f"}
        cmd = ["latexmk", "-xelatex", "-interaction=nonstopmode", "-halt-on-error", "-no-shell-escape", "doc.tex"]
        run = subprocess.run(cmd, cwd=work, env=env, capture_output=True, text=True, timeout=180)
        pdf = work / "doc.pdf"

...
| `md_parl_60` | Romanian and Russian | a Moldovan Parliament plenary session ([YouTube playlist](https://www.youtube.com/playlist?list=PLAVLfBYYrdyTHyFw9wbJz8jrxR4LGOs1a)); the hour with the most Russian next to Romanian, by Whisper-tiny language ID | public broadcast, internal testing only | none: the stenograms are not reachable by machine |
| `rompar_60` | Romanian (Romania and Moldova) | [ROMPAR](https://huggingface.co/datasets/avramandrei/rompar) test split, utterances joined in record order | not stated; internal testing only | transcript and utterance times |

...
|---|---|
| Human agency and oversight | A person reviews unconfirmed facts; in the product flow the minutes wait 60 seconds before sending and anyone can stop them (email step, outside this package). The model proposes, code checks, people decide. |
...
| Diversity, non-discrimination and fairness | Works in Romanian, Russian and English, including mixed sentences. Fluency is measured per language in the model bake-off (LanguageTool errors per 100 words) so no language gets second-class minutes. |
| Societal and environmental well-being | Runs on hardware the hospital already has; the smallest model that passes the gates is used; energy per report is still to be measured. |

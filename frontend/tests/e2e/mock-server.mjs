...
async function body(req) {
  ...
  return (req.headers["content-type"] ?? "").includes("json") && raw.length
    ? JSON.parse(raw)
    : raw;
}
...
function tick(m) {
  ...
  const step = Math.floor(
    (Date.now() - Date.parse(m.processingStartedAt)) / STEP_MS,
  );
  m.processingStages = Object.fromEntries(
    STAGE_KEYS.slice(0, step + 1).map((key, i) => [
      key,
      i < step
        ? { state: "done", endedAt: iso(-(step - i) * STEP_MS) }
        : { state: "running", startedAt: iso() },
    ]),
  );
...
function finish(m) {
  Object.assign(m, {
    ...
    actionItems: [
      {
        id: "A1",
        task: "Book the cardiac MRI",
        ownerParticipantId: "p1",
        deadline: "2026-09-28",
        completed: false,
        sourceTimestampSeconds: 184,
      },
    ],
    participants: [
      {
        ...
        ? [
            {
              id: "A2",
              kind: "action",
              text: "Order new leads.",
              problems: ["action without an owner"],
            },
          ]
        : [],
    minutesByLanguage: {
      en: {
        summary: "The board agreed a cardiac MRI before Monday's procedure.",
        decisions: [
          { id: "D1", text: "Cardiac MRI before Monday's procedure" },
        ],
        actionItems: [{ id: "A1", task: "Book the cardiac MRI" }],
      },
      ro: {
        summary:
          "Consiliul a aprobat o RMN cardiacă înainte de intervenția de luni.",
        decisions: [
          { id: "D1", text: "RMN cardiacă înainte de intervenția de luni" },
        ],
        actionItems: [{ id: "A1", task: "Programează RMN cardiacă" }],
      },
      ru: {
        summary: "Совет одобрил МРТ сердца до вмешательства в понедельник.",
        decisions: [
          { id: "D1", text: "МРТ сердца до вмешательства в понедельник" },
        ],
        actionItems: [{ id: "A1", task: "Записать на МРТ сердца" }],
      },
    },
    documents: Object.fromEntries(
      ["ro", "ru", "en"].map((l) => [
        l,
        { pdf: `MoM_${l}.pdf`, docx: `MoM_${l}.docx` },
      ]),
    ),
...
function settle(m) {
  if (
    ...
    Date.parse(m.sendScheduledAt) <= Date.now()
  ) {
    Object.assign(m, {
      status: "sent",
      deliveryState: "sent",
...
const routes = [
  ...
  [
    "GET",
    /^\/api\/capabilities$/,
    (req, res) => send(res, 200, { autoModeAvailable: true }),
  ],
  [
    "GET",
    /^\/api\/system$/,
  ...
  [
    "GET",
    /^\/api\/(people|templates|voice-profiles|speaker-clusters|action-items)$/,
    (req, res) => send(res, 200, []),
  ],
  [
    "GET",
    /^\/api\/meetings$/,
    (req, res) => send(res, 200, [...meetings.values()].map(settle)),
  ],
  [
    "POST",
    /^\/api\/meetings$/,
    async (req, res) => {
      ...
      const m = {
        ...
        sendMode: b.sendMode ?? "manual",
        createdAt: iso(),
        participants: [],
        distributionList: [`${b.type}-board`],
      };
      meetings.set(m.id, m);
      send(res, 201, m);
    },
  ...
  [
    ...
    (req, res, id) => {
      ...
      Object.assign(m, {
        ...
        processingEndsAt: iso((60 + 3600 * 0.19) * 1000),
      });
      send(res, 200, tick(m));
    },
  ],
  [
    "GET",
    /^\/api\/meetings\/([^/]+)(\/processing|\/minutes)?$/,
    (req, res, id) =>
      meetings.has(id)
        ? send(res, 200, settle(meetings.get(id)))
        : send(res, 404, {}),
  ],
  [
    "POST",
    /^\/api\/meetings\/([^/]+)\/confirmations\/([^/]+)$/,
    async (req, res, id, item) => {
      const b = await body(req);
      const m = meetings.get(id);
  ...
  [
    ...
    (req, res, id) => {
      ...
      if (m.needsConfirmation?.length)
        return send(res, 409, { error: "unresolved" });
  ...
  [
    "GET",
    /^\/api\/meetings\/([^/]+)\/documents\/(ro|ru|en)\.(pdf|docx)$/,
    (req, res, id, lang, kind) => {
      res.writeHead(200, {
        "Content-Type":
          kind === "pdf"
            ? "application/pdf"
            : "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        "Content-Disposition": `attachment; filename="MoM_${lang}.${kind}"`,
      });
...
createServer(async (req, res) => {
  ...
  if (url.pathname.startsWith("/api/")) {
    if (req.method !== "GET" && req.headers["x-requested-with"] !== "Liminal")
      return send(res, 403, { error: "csrf" });
  ...
  const target =
    file.startsWith(DIST) && extname(file) ? file : join(DIST, "index.html");
  try {
